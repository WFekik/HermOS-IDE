...
import { browserClick } from "@/lib/browser";
import { z } from "zod";
...
export const POST = withErrorHandler(async (req: NextRequest) => {
  ...
  if (!parsed.success) {
    return apiError("Invalid click payload.", 400, { details: parsed.error.flatten() });
  }

  const r = await browserClick(parsed.data.ref, user.id);
