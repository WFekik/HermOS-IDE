...
- **💻 Windows Installer Self-Healing**: The NSIS installer now terminates lingering processes rooted at the install dir (e.g. an orphaned backend holding file locks) before replacing files, and updater signatures are regenerated from final binaries with a CI verify gate — in-app updates apply cleanly instead of silently reopening the old version.
